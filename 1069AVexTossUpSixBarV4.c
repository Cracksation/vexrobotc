...
#pragma config(Motor,  port2,           backMot_right,     tmotorServoContinuousRotation, openLoop, encoder, encoderPort, dgtl1, 0)
#pragma config(Motor,  port3,           backMot_left,      tmotorServoContinuousRotation, openLoop, reversed, encoder, encoderPort, dgtl3, 1000)
#pragma config(Motor,  port4,           frontMot_right,    tmotorServoContinuousRotation, openLoop)
#pragma config(Motor,  port5,           frontMot_left,     tmotorServoContinuousRotation, openLoop, reversed)
#pragma config(Motor,  port6,           armMot_right1,     tmotorServoContinuousRotation, openLoop, encoder, encoderPort, dgtl5, 1000)
#pragma config(Motor,  port7,           armMot_right2,     tmotorServoContinuousRotation, openLoop, encoder, encoderPort, dgtl7, 1000)
#pragma config(Motor,  port8,           armMot_left1,      tmotorServoStandard, openLoop)
#pragma config(Motor,  port9,           armMot_left2,      tmotorServoStandard, openLoop)
...
//----------------------------------------------------------------------------------------------------*/
static const word armMaxPositionValue           = 220;
static const word armBuckyDumpPositionValue     = 200;
static const word armBigBallPositionValue       = 90;
static const word armBumpPositionValue          = 20;
static const word armBuckyPickupPositionValue   = 10;
...
{
    word armPositionRight       = SensorValue[armEnc_right];
    word errorArmPosRight       = setpoint - armPositionRight;

    word armPositionLeft        = SensorValue[armEnc_left];
    word errorArmPosLeft        = setpoint - armPositionLeft;

    word armGainP_right         = 1; // default gain to 1 for DOWN motion
    word armGainP_left          = 1;// default gain to 1 for DOWN motion

    word armCommandOffsetRight  = 0;
    word armCommandOffsetLeft   = 0;
    ...
    if( errorArmPosRight >= 0 )
    {
        // going up
        if( armPositionRight > 60 )
        {
            armGainP_right = 5;
            armCommandOffsetRight = 0;
        }
        else if( armPositionRight > 45 )
        {
            armGainP_right = 6;
            armCommandOffsetRight = 0;
        }
    ...
    {
        // going down
        if( armPositionRight < 10 )
        {
            armGainP_right = 2;
            armCommandOffsetRight = 0;
        }
        else if( armPositionRight < 30 )
        {
            armGainP_right = 1;
            armCommandOffsetRight = 0;
        }
    ...
    // LEFT ARM
    // set gain to higher value for UP motion
    if( errorArmPosLeft >= 0 )
    {
        // going up
        if( armPositionLeft > 60 )
        {
            armGainP_left = 5;
            armCommandOffsetLeft = 0;
        }
        else if( armPositionLeft > 45 )
        {
            armGainP_left = 6;
            armCommandOffsetLeft = 0;
        }
    ...
    {
        // going down
        if( armPositionLeft < 10 )
        {
            armGainP_left = 2;
            armCommandOffsetLeft = 0;
        }
        else if( armPositionLeft < 30 )
        {
            armGainP_left = 1;
            armCommandOffsetLeft = 0;
        }
    ...
    commandValueRight = armGainP_right * errorArmPosRight + armCommandOffsetRight;
    commandValueLeft  = armGainP_left  * errorArmPosLeft  + armCommandOffsetLeft;

    commandValueRight = (commandValueRight >  127) ?  127 : commandValueRight;
    commandValueRight = (commandValueRight < -127) ? -127 : commandValueRight;

    commandValueLeft  = (commandValueLeft >  127) ?  127 : commandValueLeft;
    commandValueLeft  = (commandValueLeft < -127) ? -127 : commandValueLeft;
...
task main ()
{
    ...
    while(1 == 1)
    {
        ...
        armBigBallCmdBtn                = vexRT[Btn7L] == 1 ? true : false;
        armBuckyPickupCmdBtn            = vexRT[Btn7D] == 1 ? true : false;
        armBuckyDumpCmdBtn              = vexRT[Btn7R] == 1 ? true : false;

        armUpOnePositionCmdBtn          = vexRT[Btn8U] == 1 ? true : false;
        armDownOnePositionCmdBtn        = vexRT[Btn8D] == 1 ? true : false;

        armUpCommand                    = vexRT[Btn8R] == 1 ? true : false;
        armDownCommand                  = vexRT[Btn8L] == 1 ? true : false;

        armUpLimitReached               = SensorValue( limitSwitch_up ) == 1 ? true : false;
        armDownLimitReached             = SensorValue( limitSwitch_down ) == 1 ? true : false;
        ...
        else if( armUpCommand && (time100[T4] != 0) )
        {
            armCtrlPositionValue++;
            ClearTimer(T4);
        }
        else if( armDownCommand && (time100[T4] != 0) )
        {
            armCtrlPositionValue--;
            ClearTimer(T4);
        }
